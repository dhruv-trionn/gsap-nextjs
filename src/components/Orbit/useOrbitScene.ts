...
export function useOrbitScene(
  ...
  useEffect(() => {
    ...
    if (!canvas || !trailCanvas || !counterEl || !progressEl) return;

    const {
      labels, images, backgroundColor, autoRotateSpeed,
      orbitRadius: R, fontFamily, heroTitle, heroSubtitle,
    } = configRef.current;
    ...
    const buildRings = () => {
      ...
      OFFSETS.forEach((off, ri) => {
        const mat = new THREE.LineBasicMaterial({ color: 0x555555, transparent: true, opacity: 0.45 });
        ...
        for (let i = 0; i <= SEG; i++) {
          ...
          if (mobile) {
            const Mr = R * 0.68;
            const mOff = off * 0.68;
            pts.push(new THREE.Vector3(mOff, Math.sin(a) * Mr, Math.cos(a) * Mr));
          } else {
        ...
        if (!mobile) {
          gsap.to(ring.position, { y: 0, duration: 1.6, delay: 0.3 + ri * 0.12, ease: 'expo.out' });
        }
        scene.add(ring);
    ...
    labels.forEach((_, i) => {
      ...
      const mat = new THREE.MeshBasicMaterial({ map: loadTex(images[i]), side: THREE.DoubleSide });
      const mesh = new THREE.Mesh(geo, mat);
      const em = new THREE.LineBasicMaterial({ color: 0x999999, transparent: true, opacity: 0.06 });
    ...
    document.fonts.ready.then(() => {
      /* "DESIGN IN" label */
      const texLabel = makeCanvasTex((cx, w, h) => {
        cx.textAlign = 'center'; cx.textBaseline = 'middle';
        cx.font = `300 90px ${fontFamily}`;
        cx.fillStyle = 'rgba(40,40,40,0.80)';
      ...
      /* "MOTION" subtitle */
      const motionTex = makeCanvasTex((cx, w, h) => {
        cx.textAlign = 'center'; cx.textBaseline = 'middle';
        cx.font = `800 380px ${fontFamily}`;
        cx.fillStyle = 'rgba(35,35,35,0.88)';
      ...
      motionMesh = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({
          map: motionTex, transparent: true, alphaTest: 0.02,
          depthTest: true, depthWrite: false, side: THREE.DoubleSide,
        }),
      );
    ...
    const setActive = (idx: number) => {
      ...
      cards.forEach((c) => {
        ...
        gsap.to(c.mesh.scale, { x: a ? 1.06 : 1, y: a ? 1.06 : 1, duration: 0.5, ease: 'power2.out' });
      });
    };

    /* ── Interaction State ── */
    let scrollRot = 0, scrollVel = 0;
    let mnx = 0, mny = 0, camX = 0, camY = 0, tiltX = 0, tiltY = 0;
    let touchY0 = 0, touchX0 = 0, touchActive = false;
    ...
    cards.forEach((c, i) => {
      ...
      gsap.to(introProgress, { [i]: 1, duration: 1.4, delay, ease: 'expo.out' });
      gsap.to(c.mesh.scale, { x: 1, y: 1, z: 1, duration: 1.2, delay, ease: 'expo.out' });
    });
    ...
    /* ── Animation Loop ── */
    let curRot = 0, tgtRot = Math.PI * 0.25, activeIdx = 0, lastTs = 0;
    ...
    const loop = (ts: number) => {
      ...
      tgtRot += scrollRot; scrollRot *= 0.88;
      curRot += (tgtRot - curRot) * 0.055;

      if (!mobile) {
        tiltX += (mnx * 0.30 - tiltX) * 0.04;
      ...
      orbitRings.forEach((r) => {
        if (!mobile) { r.rotation.z = tiltX; r.rotation.x = tiltY; }
        else { r.rotation.z = 0; r.rotation.x = 0; }
      });
      ...
      cards.forEach((c, i) => {
        ...
        if (op < 1) {
          ...
          c.mesh.position.set(orb.x * op, startY + (orb.y - startY) * op, orb.z * op);
        } else {
      ...
      let minZ = Infinity, ni = 0;
      cards.forEach((c) => { if (c.mesh.position.z < minZ) { minZ = c.mesh.position.z; ni = c.idx; } });
      if (ni !== activeIdx) { activeIdx = ni; setActive(activeIdx); }
      ...
      if (lastFrame) {
        ...
        trailCtx.drawImage(lastFrame, 0, 0, trailCanvas.width, trailCanvas.height);
    ...
    /* ── Start ── */
    gsap.fromTo(cam.position, { z: 14 }, { z: 9, duration: 2.0, ease: 'expo.out', delay: 0.1 });
    setActive(0);
    rafId = requestAnimationFrame(loop);
    ...
    return () => {
      ...
      cards.forEach((c) => {
        c.mesh.geometry.dispose(); c.mat.dispose(); c.em.dispose();
      });
      orbitRings.forEach((r) => {
        r.geometry.dispose(); (r.material as THREE.LineBasicMaterial).dispose();
      });
      [labelMesh, motionMesh].forEach((m) => {
        if (!m) return;
        m.geometry.dispose();
        const mt = m.material as THREE.MeshBasicMaterial;
        mt.map?.dispose(); mt.dispose();
      });
