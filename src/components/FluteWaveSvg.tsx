...
export default function FluteWaveSvg({
  ...
  const makeWavePath = (x1: number, y1: number, x2: number, y2: number, amp: number, phase: number, cycles: number) => {
    const segs = 26;
    const vx = x2 - x1;
    const vy = y2 - y1;
  ...
  useEffect(() => {
    ...
    for (const p of paths) {
      ...
      p.style.fill = "none";
      p.style.strokeLinecap = (p.getAttribute("stroke-linecap") || "round") as any;
      ...
      clone.addEventListener("mouseenter", () => p.dispatchEvent(new Event("mouseenter", { bubbles: true })));
    ...
    const tick = () => {
      ...
      for (const s of strings) {
        ...
        if (st.amp > 0.02 || st.speed > 0.02) {
          ...
          s.el.setAttribute("d", makeWavePath(st.x1, st.y1, st.x2, st.y2, st.amp, st.phase, st.cycles));
        }
      }
    };
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <button
        ...
        className={[
          "right-4 top-4 z-[9999] rounded-full px-4 py-2 text-[13px]",
          "border border-black/10 bg-white shadow-lg",
          "transition hover:-translate-y-[1px] hover:shadow-xl",
          soundOn ? "bg-black/[0.04] border-black/20" : "",
        ].join(" ")}
      >
        Sound: {soundOn ? "ON" : "OFF"}
      </button>

      <div className="mx-auto w-[min(1600px,120vw)] p-6">
